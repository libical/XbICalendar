//
//  XbICalendarProcessCalendarTests.m
//  XbICalendar
//
...
@interface XbICalendarProcessCalendarTests : XbICalendarIcsTest

@end

@implementation XbICalendarProcessCalendarTests
...
{
    XbICVEvent * vEvent1 = [self componentAtIndex:0 kind:ICAL_VEVENT_COMPONENT ofCalendarAtIndex:0];
    [self assertUtcDateString:@"19970611T000000Z"
                isEqualToDate:[vEvent1 dateStamp]
           withFailureMessage:@"event dateStamp is incorrect"];
    
    XbICVEvent * vEvent2 = [self componentAtIndex:0 kind:ICAL_VEVENT_COMPONENT ofCalendarAtIndex:1];
    [self assertUtcDateString:@"19970611T010000Z"
...
- (void)test_events_with_summary
{
    ...
    XbICVEvent * vEvent1 = [self componentAtIndex:0 kind:ICAL_VEVENT_COMPONENT ofCalendarAtIndex:0];
    XCTAssertEqualObjects([vEvent1 summary], @"Event 1", @"event summary is incorrect");
    
    XbICVEvent * vEvent2 = [self componentAtIndex:0 kind:ICAL_VEVENT_COMPONENT ofCalendarAtIndex:1];
    XCTAssertEqualObjects([vEvent2 summary], @"Event 2", @"event summary is incorrect");
    
    XbICVEvent * vEvent3 = [self componentAtIndex:0 kind:ICAL_VEVENT_COMPONENT ofCalendarAtIndex:2];
    XCTAssertEqualObjects([vEvent3 summary], @"Event 3", @"event summary is incorrect");
}

- (void)test_event_with_uid
{
    XbICVEvent * vEvent1 = [self componentAtIndex:0 kind:ICAL_VEVENT_COMPONENT ofCalendarAtIndex:0];
    XCTAssertEqualObjects([vEvent1 UID], @"calsrv.example.com-873970198738700@example.com", @"Unexpected event UID");
    
    XbICVEvent * vEvent2 = [self componentAtIndex:0 kind:ICAL_VEVENT_COMPONENT ofCalendarAtIndex:1];
    XCTAssertEqualObjects([vEvent2 UID], @"calsrv.example.com-873970198738701@example.com", @"Unexpected event UID");
    
    XbICVEvent * vEvent3 = [self componentAtIndex:0 kind:ICAL_VEVENT_COMPONENT ofCalendarAtIndex:2];
    XCTAssertEqualObjects([vEvent3 UID], @"calsrv.example.com-873970198738702@example.com", @"Unexpected event UID");
}
